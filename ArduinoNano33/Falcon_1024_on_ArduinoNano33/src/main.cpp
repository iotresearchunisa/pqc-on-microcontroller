...
test_codec(void)
{
	...
	Serial.print("Test encode/decode:");				
	fflush(stdout);
	tlen = 8192;
	tmp = (uint8_t *) xmalloc(tlen);
	...
	for (ctr = 0; ctr < num; ctr ++) {
		...
		for (u = 0; u < n; u ++) {
			if (fpr_rint(g[u]) != fpr_rint(h[u])) {
				Serial.println("split/merge error");				
				exit(EXIT_FAILURE);
			}
		}
...
check_gaussian0_sampler(const u72 *x, int r)
{
	...
	if (z != r) {
		...
		Serial.println("wrong half-Gaussian output");
...
test_gaussian0_sampler(void)
{
	...
	Serial.print("Test half-Gaussian sampler: ");
	...
	if (u72_add_small(&t, 1) != 1) {
		Serial.println(" wrong distribution sum.");
		exit(EXIT_FAILURE);
	}
...
#define NUM_SAMPLES   100000
	...
	for (ctr = 0; ctr < NUM_SAMPLES; ctr ++) {
		...
		if (z < -MAX_DEV || z > +MAX_DEV) {
			Serial.println("out-of-range sampled value");
			exit(EXIT_FAILURE);
		}
		zz[z + MAX_DEV] ++;
	}
	...
	if (df == 13) {
		if (!fpr_lt(chi, chi_dt_13)) {
			Serial.println("chi-square test fail");
			exit(EXIT_FAILURE);
		}
	} else if (df == 14) {
		...
		Serial.println("unexpected number of classes:");
		exit(EXIT_FAILURE);
	}
...
bool measureTime = true;  // Imposta a true per misurare il tempo, false per misurare i cicli di clock
...
void setup() {
    ...
    unsigned long startTime, endTime, elapsedTime_test_nist_KAT;
    ...
    if (measureTime) {
        // Misurazione del tempo in microsecondi
        startTime = micros();
      	test_nist_KAT(10, "affdeb3aa83bf9a2039fa9c17d65fd3e3b9828e2");
        endTime = micros();
        elapsedTime_test_nist_KAT = endTime - startTime;

        // Risultato del tempo
        Serial.println("");
        Serial.print("Time Falcon 1024: ");
        Serial.print(elapsedTime_test_nist_KAT);
        Serial.println(" µs");
    } else {
        // Misurazione dei cicli di clock
        //CLOCK1 = ESP.getCycleCount();
      	test_nist_KAT(10, "affdeb3aa83bf9a2039fa9c17d65fd3e3b9828e2");

        //CLOCK2 = ESP.getCycleCount();

        CLOCK_test_nist_KAT = CLOCK2 - CLOCK1;

        // Risultato dei cicli di clock
        Serial.println("");
        Serial.print("Cycle Count Falcon 1024: ");
    }
