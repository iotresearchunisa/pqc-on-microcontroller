...
void vect_mul(uint64_t *o, const uint64_t *a1, const uint64_t *a2) {
    // Allocazione della memoria con malloc
    uint64_t *stack = (uint64_t *)malloc((VEC_N_SIZE_64 << 3) * sizeof(uint64_t));
    
    // Verifica se l'allocazione della memoria è andata a buon fine
    if (stack == NULL) {
        //Serial.println("Memory allocation failed for stack");
        return;
    }

    // Dichiarazione dell'array per il risultato intermedio
    uint64_t o_karat[(VEC_N_SIZE_64 << 1) + 1] = {0};

    // Esegui la moltiplicazione di Karatsuba e la riduzione
